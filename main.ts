...
serve(async (request) => {
  ...
  if (pathname.startsWith("/proxy/")) {
    try {
      ...
      // 允许的请求头列表
      const allowedHeaders = ["accept", "content-type", "authorization", "accept-encoding", "accept-language", "cache-control", "pragma", "x-requested-with", "x-forwarded-for", "x-real-ip"]; // Added common proxy related headers
      ...
      // 通用代理模式下的 User-Agent 处理
      // 如果客户端请求带有 User-Agent，则转发客户端的 User-Agent
      // 否则，生成一个随机的 User-Agent
  ...
  try {
    const headers = new Headers();
    // Forward specific headers, 移除了 anthropic-version
    const commonApiHeaders = ["content-type", "authorization", "accept"];
    request.headers.forEach((value, key) => {
        if (commonApiHeaders.includes(key.toLowerCase()) || key.toLowerCase().startsWith("x-") || key.toLowerCase().startsWith("http-")) { // 添加 http- 以兼容openrouter
            headers.set(key, value);
        }
    });
    
    // API 代理模式下的 User-Agent 处理 - 总是使用随机生成的 User-Agent
    headers.set("User-Agent", getRandomUserAgent());
    ...
    responseHeaders.set("Access-Control-Allow-Origin", "*");
    responseHeaders.set("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE, OPTIONS, HEAD, PATCH");
    // 更新 Access-Control-Allow-Headers 以匹配转发的头
